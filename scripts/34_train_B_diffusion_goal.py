#!/usr/bin/env python3
"""Train Policy B with Diffusion Policy using goal-based actions.

This script trains a Diffusion Policy for Task B (reverse pick-and-place) using
data collected by script 14 (14_collect_B_with_goal_actions.py).

=============================================================================
KEY DIFFERENCE FROM SCRIPT 31
=============================================================================
Script 31 (train_A_diffusion.py):
    - Action = next frame's ee_pose + gripper (smooth interpolated)
    
Script 34 (this script):
    - Action = FSM goal position (sharp, decisive)
    - Uses the "action" field directly from script 14's output

This produces a policy that outputs more decisive goal positions, which may
lead to faster and more accurate task execution.

=============================================================================
INPUT DATA FORMAT (from script 14_collect_B_with_goal_actions.py)
=============================================================================
NPZ file with episodes list, each dict containing:
    ...
    - action:        (T, 8)   Goal actions [x, y, z, qw, qx, qy, qz, gripper]
    - gripper:       (T,)     Gripper actions (+1=open, -1=close)
    - fsm_state:     (T,)     FSM state at each timestep
    - place_pose:    (7,)     Place position
    - goal_pose:     (7,)     Goal position (cube start)
...
The script converts the NPZ data to LeRobot v3.0 format:
    ...
    - observation.wrist_image: (C, H, W) float32 wrist camera images
    - observation.state: (7,) or (14,) float32 end-effector pose (+ obj_pose)
    - action: (8,) float32 goal action [ee_pose(7), gripper(1)]

=============================================================================
USAGE EXAMPLES
=============================================================================
# Step 1: Convert data only (recommended for first run)
CUDA_VISIBLE_DEVICES=2 python scripts/34_train_B_diffusion_goal.py \
    --dataset data/B_2images_goal.npz \
    --out runs/diffusion_B_goal \
    --convert_only \
    --include_obj_pose
...
CUDA_VISIBLE_DEVICES=2 python scripts/34_train_B_diffusion_goal.py \
    --dataset data/B_2images_goal.npz \
    --out runs/diffusion_B_goal_test \
    --steps 1000 \
    --batch_size 8

# Multi-GPU training
CUDA_VISIBLE_DEVICES=1,2,3,4,5,6 torchrun --nproc_per_node=6 \
    scripts/34_train_B_diffusion_goal.py \
    --dataset data/B_2images_goal.npz \
    --out runs/diffusion_B_goal \
    --batch_size 64 \
    --steps 5000 \
...
def convert_npz_to_lerobot_format(
    ...
    print(f"  FPS: {fps}")
    print(f"  Action type: FSM goal position (from 'action' field)")
    ...
    for ep_idx, ep in enumerate(episodes):
        ...
        # =====================================================================
        # In script 31: action = next_ee_pose + gripper (smooth)
        # Here: action = ep["action"][t] (FSM goal position - sharp)
        for t in range(T):
            # Current observation
            img = images[t]  # (H, W, 3)
            if include_obj_pose:
