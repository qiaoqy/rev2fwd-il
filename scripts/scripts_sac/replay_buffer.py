...
Reuses the same design as scripts_rl/replay_buffer.py but with
SAC-specific additions (log_prob storage for diagnostics).
"""
...
class ReplayBuffer:
    ...
    Pre-allocates numpy arrays to avoid repeated allocation during training.
    Returns PyTorch tensors when sampled.
    """
    ...
    def sample(self, batch_size: int, device: str = "cuda") -> Dict[str, torch.Tensor]:
        """Sample a random batch and return as device tensors."""
...
class DemoBuffer:
    ...
    Read-only — data is loaded once and sampled repeatedly for BC regularization.
    """
