...
This script analyzes the rollout_record_baseline.json file generated by the iterative training
pipeline and creates visualizations to understand finetuning effectiveness.

Key Metrics:
-----------
1. Average Steps per Successful Task (主要指标):
   - 衡量成功完成一个任务平均需要的步数
   - 计算方式: total_success_steps / num_completed_tasks
   - 任务失败已体现在 num_completed_tasks 中（失败越多，完成数越少）
   - 步数越少 = 模型执行效率越高
   - 越低越好

2. Task-specific Success Steps:
   - 分别追踪 Task A 和 Task B 的平均成功步数
   - 步数越少 = 执行效率越高
...
------
    ...
    python scripts/scripts_pick_place/8_analyze_rollout_record.py --record data/rollout_record_baseline.json
...
def plot_effort_analysis(
    ...
    # Plot 3: Consecutive Successes (A→B cycles)
    ax3 = axes[1, 0]
    ...
    ax3.set_ylabel('Consecutive A→B Cycles', fontsize=11)
...
def print_summary(analysis_results: list, record: dict):
    ...
    if len(finite_results) >= 3:
        ...
        print(f"\nTrend Analysis:")
        if slope < -1:
            print(f"  📉 Improving (slope={slope:.2f}): Model is getting more efficient")
        elif slope > 1:
            print(f"  📈 Degrading (slope={slope:.2f}): Model performance may be declining")
        else:
            print(f"  ➡️ Stable (slope={slope:.2f}): Performance is relatively constant")
    
    # Identify issues
    print(f"\nPotential Issues:")
    no_B_success = sum(1 for r in analysis_results if r['success_count_B'] == 0)
    if no_B_success > len(analysis_results) * 0.5:
        print(f"  ⚠️ Task B rarely succeeds ({no_B_success}/{len(analysis_results)} iterations)")
    ...
    if no_consecutive > len(analysis_results) * 0.7:
        print(f"  ⚠️ Low consecutive successes ({no_consecutive}/{len(analysis_results)} iterations with 0)")
        print(f"     The A→B→A loop often fails on first B attempt")
...
Examples:
  ...
  python scripts/scripts_pick_place/8_analyze_rollout_record.py --record data/rollout_record_baseline.json --out data/analysis
        """
    )
    parser.add_argument('--record', type=str, default='data/rollout_record_baseline.json',
                        help='Path to rollout_record_baseline.json file')
