#!/bin/bash
# =============================================================================
# Ablation Baseline: Train with SAME Data Only (No Rollout Aggregation)
# =============================================================================
#
# Control experiment for run_pipeline.sh.
# Same evaluate → train → repeat loop, but does NOT add successful rollout
# data to the training set. Isolates the effect of additional training steps
# from the benefit of new data aggregation.
#
# Compare this baseline's success rate curve with run_pipeline.sh to determine
# whether improvements come from:
#   (a) new rollout data (DAgger effect), or
...
# Configuration
# =============================================================================
MODE="ablation"             # ablation = NO rollout data aggregation (baseline)
...
json.dump({
    ...
    'note': 'ABLATION: No rollout data aggregation. Training uses original dataset only.',
...
import json
json.dump({
    'description': 'Ablation baseline: train with original data only (no DAgger)',
...
# =============================================================================
# Train One Policy (A or B) — ABLATION: original data only, no rollout merge
# =============================================================================
train_policy() {
    local NAME=$1        # "A" or "B"
    ...
    # --- Prepare dataset + checkpoint (NO rollout data — ablation) ---
    echo "  Using original dataset only (ablation: no rollout aggregation)"
...
echo "  Mode:       $MODE (no rollout data aggregation)"
...
for iter in $(seq 1 $MAX_ITERATIONS); do
    print_header "Iteration $iter / $MAX_ITERATIONS (ABLATION)"
...
"
    ...
    # ---- Phase 2: Train Policy A (no rollout data) ----
    if ! phase_done $iter train_A; then
        echo "--- Train Policy A ($STEPS_PER_ITER steps, original data only) ---"
        train_policy A "$PA_TEMP" "$PA_LAST"
        mark_done $iter train_A
    else
        echo "  [Train A] Already done — skipping"
    fi

    # ---- Phase 3: Train Policy B (no rollout data) ----
    if ! phase_done $iter train_B; then
        echo "--- Train Policy B ($STEPS_PER_ITER steps, original data only) ---"
        train_policy B "$PB_TEMP" "$PB_LAST"
...
echo ""
echo "  NOTE: This ablation used ORIGINAL data only (no DAgger aggregation)."
echo "  Compare with a 'pipeline' experiment to measure the DAgger effect."
echo ""
...
if iters:
    print("  ABLATION Results (no new data):")
