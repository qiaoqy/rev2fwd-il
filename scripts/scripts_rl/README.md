...
$$r_t(\theta) = \frac{\pi_\theta(\text{action}_t \mid \text{state}_t)}{\pi_{\theta_{\text{old}}}(\text{action}_t \mid \text{state}_t)}$$
...
$$r_k(\theta) = \exp\left( \log \pi_\theta(\epsilon_k \mid x_k, k, s) - \log \pi_{\theta_{\text{old}}}(\epsilon_k \mid x_k, k, s) \right)$$
...
| $\mathcal{L}_{KL}$ | $\lVert\epsilon_\theta^{\text{new}} - \epsilon_\theta^{\text{ref}}\rVert^2$ | 约束去噪输出不偏离 BC 预训练太远 |
...
$$\mathcal{L}_{actor} = -Q_{\phi}(s, \pi_\theta(s)) + \lambda \cdot \mathbb{E}_{(s,a)\sim D_{demo}} \left[ \lVert \epsilon_\theta(x_t, t, s) - \epsilon \rVert^2 \right]$$

> 在示范数据上保持扩散训练 loss，让 policy 不忘记"专家是怎么做的"

**方案 B (MSE KL Penalty)**:

$$\mathcal{L}_{KL} = \mathbb{E}_{x_k, k, s} \left[ \lVert \epsilon_\theta(x_k, k, s) - \epsilon_{ref}(x_k, k, s) \rVert^2 \right]$$

> 对于相同输入，比较当前网络和冻结预训练网络的输出差异。差异越大，惩罚越重。
>
> 与真正的 KL 散度 $D_{KL}(\pi_\theta \,\Vert\, \pi_{ref})$ 的关系:
