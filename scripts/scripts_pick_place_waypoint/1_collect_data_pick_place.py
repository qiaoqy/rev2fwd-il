...
def rollout_expert_B_with_goal_actions(
    ...
    action_lists = [[] for _ in range(num_envs)]          # Next-frame ee_pose as action
    ...
    for t in range(horizon):
        ...
        # Record data for each env (action will be computed later using next frame's ee_pose)
        ...
        # Get gripper command from expert for this step
        # We'll use this for the action's gripper component
        goal_action = get_fsm_goal_action(expert, object_pose)
        gripper_cmd_np = goal_action[:, 7].cpu().numpy()  # Extract gripper command
        ...
        for i in range(num_envs):
            ...
            # Action placeholder - will be computed after loop using next frame's ee_pose
            gripper_lists[i].append(gripper_cmd_np[i])
    ...
    for i in range(num_envs):
        ...
        ee_pose_arr = np.array(ee_pose_lists[i], dtype=np.float32)  # (T, 7)
        gripper_arr = np.array(gripper_lists[i], dtype=np.float32)  # (T,)
        
        # =====================================================================
        # Compute action as next frame's ee_pose + current gripper command
        # For frame t, action = [ee_pose[t+1], gripper[t]]
        # Last frame is dropped since there's no next frame for it
        # =====================================================================
        T = len(ee_pose_arr)
        if T > 1:
            # Action = next frame's ee_pose (7D) + current gripper (1D)
            action_arr = np.zeros((T - 1, 8), dtype=np.float32)
            action_arr[:, :7] = ee_pose_arr[1:]  # next frame's ee_pose
            action_arr[:, 7] = gripper_arr[:-1]   # current gripper command
        else:
            # Edge case: single frame episode
            action_arr = np.zeros((0, 8), dtype=np.float32)
        
        episode_dict = {
            "obs": np.array(obs_lists[i], dtype=np.float32)[:-1] if T > 1 else np.array(obs_lists[i], dtype=np.float32),
            "images": np.array(image_lists[i], dtype=np.uint8)[:-1] if T > 1 else np.array(image_lists[i], dtype=np.uint8),  # (T-1, H, W, 3)
            "wrist_images": np.array(wrist_image_lists[i], dtype=np.uint8)[:-1] if T > 1 else np.array(wrist_image_lists[i], dtype=np.uint8),  # (T-1, H, W, 3)
            "ee_pose": ee_pose_arr[:-1] if T > 1 else ee_pose_arr,  # (T-1, 7)
            "obj_pose": np.array(obj_pose_lists[i], dtype=np.float32)[:-1] if T > 1 else np.array(obj_pose_lists[i], dtype=np.float32),  # (T-1, 7)
            "action": action_arr,  # (T-1, 8) - next frame ee_pose + gripper
            "gripper": gripper_arr[:-1] if T > 1 else gripper_arr,  # (T-1,)
            "fsm_state": np.array(fsm_state_lists[i], dtype=np.int32)[:-1] if T > 1 else np.array(fsm_state_lists[i], dtype=np.int32),  # (T-1,)
...
def save_episodes_with_goal_actions(path: str, episodes: list[dict]) -> None:
    ...
    if episodes:
        ...
        print(f"  - Action shape (next ee_pose + gripper): {ep0['action'].shape}")
