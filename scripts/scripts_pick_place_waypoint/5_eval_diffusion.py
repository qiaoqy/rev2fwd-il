...
def run_episode(
    ...
    Args:
        ...
        include_gripper: Whether the policy expects gripper state in obs.
    ...
    for t in range(horizon):
        ...
        if include_obj_pose:
            ...
            if include_gripper:
                # Use last_gripper_state which tracks the gripper command
                gripper_tensor = torch.tensor([[last_gripper_state]], device=device)  # (1, 1)
                state = torch.cat([ee_pose, obj_pose, gripper_tensor], dim=-1)  # (1, 15)
            else:
        ...
        # action shape: (num_envs, 8), gripper is the last dimension
...
def main() -> None:
    ...
    try:
        ...
        print(f"  Includes gripper in state: {include_gripper}")
