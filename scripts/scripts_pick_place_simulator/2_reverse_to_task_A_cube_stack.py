#!/usr/bin/env python3
"""Build Task A (stack) data by time-reversing Task B (unstack) trajectories.

Adapted from 2_reverse_to_task_A.py for the exp38 3-cube format.
Handles extra per-cube pose arrays, fsm_round, place_targets, etc.

Usage:
    conda activate rev2fwd_il
    python scripts/scripts_pick_place_simulator/2_reverse_to_task_A_cube_stack.py \
        --input data/pick_place_isaac_lab_simulation/exp38/task_B_cube_stack_100.npz \
        --out data/pick_place_isaac_lab_simulation/exp38/task_A_cube_stack_100.npz --verify
"""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Time-reverse Task B cube-stack data to get Task A training data.",
    )
...
def reverse_episode(ep: dict) -> dict:
    """Reverse an episode in time for the 3-cube stack task."""
    T = len(ep["images"])
    ...
    # Ensure float32 for numeric temporal arrays
    for key in ["obs", "ee_pose", "gripper",
                "cube_large_pose", "cube_medium_pose", "cube_small_pose", "obj_pose"]:
