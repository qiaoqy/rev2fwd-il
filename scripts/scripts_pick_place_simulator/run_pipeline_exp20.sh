#!/bin/bash
# =============================================================================
# Rev2Fwd Pick-and-Place Simulator Pipeline — Exp20 variant
# =============================================================================
#
# 与 run_pipeline.sh 的区别 (参见 exp20/plan.md):
#   1. 使用对侧策略 rollout 的倒放数据训练 (避免 self imitation)
#      - Policy A 使用 Policy B rollout 的倒放
#      - Policy B 使用 Policy A rollout 的倒放
#   2. 训练数据不累积, 每次只用: 初始数据 + 最新一轮倒放 rollout
#   3. 去掉采样权重 (WeightedRandomSampler), 均匀采样
#   4. Task B 成功判定区域不缩减 (cube_half_size=0, 已在
#      6_test_alternating.py 中修改)
#
# =============================================================================
...
# =============================================================================
# Key differences from run_resume_train in run_pipeline.sh:
#   1. Uses ORIGINAL lerobot dataset (not accumulated $TEMP/lerobot_dataset)
#   2. Does NOT use weighted sampling (uniform dataloader sampling)
#   3. Takes ORIG_LEROBOT as an explicit parameter
# =============================================================================
run_fresh_train() {
    # Usage: run_fresh_train <name> <temp_dir> <last_dir> <rollout_npz_or_empty> <orig_lerobot> <extra_steps>
    local NAME=$1
    local TEMP=$2
    local LAST=$3
    local ROLLOUT=$4
    ...
    # Prepare dataset: ALWAYS start from ORIGINAL lerobot + current reversed rollout only
    # (no accumulation of historical rollouts)
    local ROLLOUT_ARG=""
    ...
    # NOTE: Do NOT copy wandb state — exp20 uses a fresh wandb project
    # Old wandb run IDs from exp17 would cause resume errors
    ...
    # NO weighted sampling — uniform dataloader sampling
    local PORT
    PORT=$(random_port)
...
print_header "Rev2Fwd Pick-and-Place Pipeline (Exp20 variant)"
...
echo "  Variant:     reverse-opposite rollout, non-cumulative, uniform sampling"
echo ""

# =============================================================================
# Phase 1: Data Preparation (expected to be pre-populated for exp20)
# =============================================================================
...
json.dump({
    'description': 'Rev2Fwd Pick-and-Place iterative training (Exp20 variant: reverse-opposite, non-cumulative, uniform sampling)',
    'config': {
        'exp_name': '$EXP_NAME',
        'variant': 'exp20',
        ...
        'changes': [
            ...
            'uniform_sampling: No WeightedRandomSampler, uniform dataloader',
            'task_B_no_shrink: cube_half_size=0 in success check',
        ],
    },
    'iterations': []
...
# =============================================================================
# Phase 3: Iterative DAgger (Exp20 variant)
# =============================================================================
print_header "Phase 3: Iterative Training ($ITER_ROUNDS rounds, Exp20 variant)"
...
"
    ...
    # --- Step 3.1.5: Reverse OPPOSITE rollout data ---
    # Policy A trains on reversed B rollout; Policy B trains on reversed A rollout
    ROLLOUT_B_REVERSED="${BASE_DIR}/iter${iter}_collect_B_reversed.npz"
    ROLLOUT_A_REVERSED="${BASE_DIR}/iter${iter}_collect_A_reversed.npz"

    if ! phase_done "iter${iter}_reverse"; then
        log "Reversing opposite rollout data..."

        if [ -f "$ROLLOUT_B" ]; then
            log "  Reversing B rollout → ${ROLLOUT_B_REVERSED} (for Policy A)"
            python "${SCRIPT_DIR}/2_reverse_to_task_A.py" \
                --input "$ROLLOUT_B" \
                --out "$ROLLOUT_B_REVERSED" \
                --success_only 1 \
                --verify \
                2>&1 | tee "${LOG_DIR}/iter${iter}_reverse_B.log"
        else
            log "  WARNING: $ROLLOUT_B not found, skipping B reversal"
        fi

        if [ -f "$ROLLOUT_A" ]; then
            log "  Reversing A rollout → ${ROLLOUT_A_REVERSED} (for Policy B)"
            python "${SCRIPT_DIR}/2_reverse_to_task_A.py" \
                --input "$ROLLOUT_A" \
                --out "$ROLLOUT_A_REVERSED" \
                --success_only 1 \
                --verify \
                2>&1 | tee "${LOG_DIR}/iter${iter}_reverse_A.log"
        else
            log "  WARNING: $ROLLOUT_A not found, skipping A reversal"
        fi

        mark_done "iter${iter}_reverse"
    else
        log "[Skip] Iteration $iter reversal already done"
    fi

    # --- Step 3.2: Train Policy A ---
    # Uses ORIGINAL lerobot/task_A + reversed B rollout (non-cumulative)
    if ! phase_done "iter${iter}_train_A"; then
        log "Training Policy A with reversed B rollout ($STEPS_PER_ITER steps)..."
        run_fresh_train A "$PA_TEMP" "$PA_LAST" "$ROLLOUT_B_REVERSED" "${LEROBOT_DIR}/task_A" $STEPS_PER_ITER
    ...
    # --- Step 3.3: Train Policy B ---
    # Uses ORIGINAL lerobot/task_B + reversed A rollout (non-cumulative)
    if ! phase_done "iter${iter}_train_B"; then
        log "Training Policy B with reversed A rollout ($STEPS_PER_ITER steps)..."
        run_fresh_train B "$PB_TEMP" "$PB_LAST" "$ROLLOUT_A_REVERSED" "${LEROBOT_DIR}/task_B" $STEPS_PER_ITER
